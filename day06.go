...
func NewDay06() Day {
	d := &Day06{
		declarations: Day06Content,
	}
	return d
}

func (d *Day06) SolvePart1() {
	fullYesCount := 0
	for _, groupDeclarations := range strings.Split(d.declarations, "\n\n") {
		fullYesCount += d.countAnyYes(groupDeclarations)
	}
	fmt.Printf("total 'any' yes counts %d\n", fullYesCount)
}

func (d *Day06) SolvePart2() {
	fullYesCount := 0
	for _, groupDeclarations := range strings.Split(d.declarations, "\n\n") {
...
const Day06Content = `necytxmlfhsu
uecosjvlhpmk
