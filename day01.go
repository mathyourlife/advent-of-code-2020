...
func (d *Day01) findThreeExpenseEntries() (int, int, int) {
	// even more brute force since it still isn't a long list
	for i := 0; i < len(d.expenses)-2; i++ {
		for j := i + 1; j < len(d.expenses); j++ {
			for k := j + 1; k < len(d.expenses); k++ {
				if d.expenses[i]+d.expenses[j]+d.expenses[k] == 2020 {
					return d.expenses[i], d.expenses[j], d.expenses[k]
				}
			}
		}
	}
